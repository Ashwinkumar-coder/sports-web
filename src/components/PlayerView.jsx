  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-lg text-center">
          <div className="text-2xl font-black text-purple-400">{dashboardData.overall_performance.toFixed(1)}</div>
      ...
      <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-lg space-y-4">
        ...
        <p className="text-slate-400 text-sm">Join an active cricket tournament by naming your team squad, selecting a certified Coach, and nominating team players.</p>
        
        {tournaments.filter(t => t.is_approved && t.status === 'registration_open').length === 0 ? (
          <p className="text-slate-500 text-sm italic">No open tournaments available for registration at this time.</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Tournaments list */}
            <div className="space-y-2">
              {tournaments.filter(t => t.is_approved && t.status === 'registration_open').map(t => (
                <div
                  key={t.id}
                  onClick={() => {
                ...
                >
                  ...
                  <div className="text-xs text-slate-400">Entry fee: ${t.fee} | Team size: {t.maximum_player_count} players</div>
                </div>
              ))}
            </div>
